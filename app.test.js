...
describe("getCommandLineArguments", () => {
  test("returns command line arguments for --filter=ry", () => {
    process.argv = ['node', 'app.js', '--filter=ry'];
    expect(getCommandeLineArguments()).toEqual(['--filter=ry']);
  });

  test("returns command line arguments when --count is present", () => {
    process.argv = ['node', 'app.js', '--count'];
    expect(getCommandeLineArguments()).toEqual(['--count']);
  });
});

describe("filterAnimalsByPattern", () => {
  test("test filterAnimalsByPattern with 'ry'", () => {
    const testData =[
      {
        "name": "Uzuzozne",
        "people": [
    ...
    const result = filterAnimalsByPattern(data, "ry");
    expect(result).toEqual(testData);
  });

  test("filterAnimalsByPattern with a non-existing pattern", () => {
    const result = filterAnimalsByPattern(data, "nonexistent");
    expect(result).toEqual([]);
  });

  test("filterAnimalsByPattern with an empty pattern", () => {
    const result = filterAnimalsByPattern(data, "");
    expect(result).toEqual(data);
  });
});


describe("updateNamesWithCounts", () => {
  test("test updateNamesWithCounts", () => {
    const testData = [
      {
        "name": "Dillauti [5]",
        "people": [
  ...
  const result = updateNamesWithCounts(data);
  expect(result).toEqual(testData);
  });
});
